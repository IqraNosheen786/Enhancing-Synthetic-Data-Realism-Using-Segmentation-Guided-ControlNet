...
print(torch.cuda.get_device_name(0))  # To check the GPU being used
import os
...
def main(
    *,
    ...
    num_images_to_train: int = 1000,
    pretrained_model_path: str = 'SG161222/Realistic_Vision_V2.0',#"dreamlike-art/dreamlike-photoreal-2.0",
    lora_rank: int = 4,
    output_dir: str = "./Realistic_Vision_V2.0",
    data_root: str = '/data/iqra/datasets/kitti/data_object_image_3/training/',  
    seed: Optional[int] = 0,
    learning_rate: float = 1e-5,
    train_batch_size: int = 6,
    max_train_steps: int = 22000,
    ...
    validation_prompt: str = "A kitti_scene of roads with vehicles, pedestrians, road signs, grass, trees, and buildings, captured from a moving vehicle under various environmental and weather conditions.",  # Updated for urban scenes
    guidance_scale: float = 9.0,
    negative_prompt: str = "cartoon, low resolution, low quality, unrealistic, distorted, abstract, extreme weather, blurry, overexposed, futuristic vehicles, animals, flying objects, underwater scenes, fantasy elements, alien landscapes, incorrect proportions, scifi buildings, overly bright colors, artistic effects, long exposure, unnatural lighting, distorted vehciles, blurry cars, out-of-focus vehicles, distorted vehicles, low-detail cars, low-resolution cars, incorrect road signs, abstract signs, low-detail signs, unrealistic road signs, distorted text on signs, cartoon grass, fake grass, unrealistic vegetation, overly bright grass, low-texture grass, artificial grass, 2D grass, heavy shadows, overly dark shadows, unnatural lighting, harsh shadows, extreme shadow contrast, underexposed."
):

    # Initialize WandB
    wandb.init(project="LORA_Training", config={
        "exp_name": exp_name,
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    #unet.requires_grad_(True)  # Enable gradients for the model
    #unet.enable_input_require_grads()  # Ensure input tensors require gradients
    ...
    '''
    for name, param in lora_attn_procs.items():
    ...
    num_workers=num_workers  # Adjust based on your system's CPU cores
    )
    ...
    def get_input(batch, encode_image=True, encode_prompt=True):
        ...
        x = x.to(device=device, memory_format=torch.contiguous_format,non_blocking=True)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                #prompt_embed.requires_grad = True
                ...
                prompt_embed = prompt_embed.to(device=accelerator.device, dtype=weight_dtype, non_blocking=True)
                ...
                lr_scheduler.step()
                optimizer.zero_grad()
            # Log every 100 steps to avoid excessive logging
            if global_step % 1000 == 0:
                logs = {"step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
                progress_bar.set_postfix(**logs)
                wandb.log({"train/loss": train_loss, "learning_rate": lr_scheduler.get_last_lr()[0]}, step=global_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="/users/iqra_n/diffusion/Kitti_Lora/ST_LORA/configs/my_experiment.yaml")
